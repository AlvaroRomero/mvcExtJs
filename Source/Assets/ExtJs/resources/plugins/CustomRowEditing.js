Ext.define('Ext.ux.CustomRowEditing', {
	...
	init: function (grid) {
		...
		grid.allowBlank = typeof grid.allowBlank == 'undefined' ? true : grid.allowBlank;
		grid.validateRecords = typeof grid.validateRecords == 'undefined' ? false : grid.validateRecords;
		...
		grid.view.on('itemupdate', function() {
			me.addEmptyRow();
		});
		grid.view.on('itemremove', function() {
			me.addEmptyRow();
		});

		if(!grid.validate) {
			grid.validate = Ext.emptyFn;
		}
	...
	validate: function (validationRecord) {
		...
		if (!grid.allowBlank && grid.store.getCount() == 1 && grid.store.getAt(0) == this.rowEditingModel) {
			...
			row.dom.setAttribute('data-errorqtip', 'Debe agregar al menos una partida');
