...
Ext.define('Ext.ux.Router', {
			...
			for (; i < len; i++) {
				...
				if (route.regex) {
					...
					if (matches) {
						...
						if (me.dispatch(token, route, Ext.apply({ routerRedirected: this.routerRedirected }, matches))) {
							this.routerRedirected = false;
					...
					if (matches) {
						...
						if (matches && me.dispatch(token, route, Ext.apply({ routerRedirected: this.routerRedirected }, params))) {
							this.routerRedirected = false;
							return params;
						}
					}
		...
		dispatch: function(token, route, params) {
			...
			controller[route.action].call(controller, params, route.action, token, route);
